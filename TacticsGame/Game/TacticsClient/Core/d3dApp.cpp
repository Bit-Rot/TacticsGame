...
void D3DApp::initApp()
{
	...
	wcscpy_s(fontDesc.FaceName, 16*sizeof(wchar_t), L"Times New Roman");
...
void D3DApp::initDirect3D()
{
	...
	*/
	...
	//Create an adapter before creating a device so we can pass it to CreateDevice.
	//This forces the device created and the swap chain we create later to belong
	//to the same factory.  If we create the device passing in NULL for our adapter,
	//the adapter is created with a separate adapter than our swap chain, which will
	//issue an error.
	HR(CreateDXGIFactory(__uuidof(IDXGIFactory), (void **)(&mdxgiFactory)));
	IDXGIAdapter *dxgiAdapter = NULL;
	mdxgiFactory->EnumAdapters(0, &dxgiAdapter);

	//Create ONLY a device as part of this exercise, create swap chain later.
	...
	//Create a Swap Chain
	HR(mdxgiFactory->CreateSwapChain(md3dDevice, &sd, &mSwapChain));

	//Disable default 'Alt+Enter --> Toggle Fullscreen' functionality
	mdxgiFactory->MakeWindowAssociation(mhMainWnd, DXGI_MWA_NO_WINDOW_CHANGES);
